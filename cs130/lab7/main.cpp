	...
	while(true){
		...
		for(int x = XMIN; x <= XMAX; ++x){
			for(int y = YMIN; y <= YMAX; ++y){
				...
				vector3 color(255,0,0); // fill this in with the appropriate colors
				...
				//calculate d
				vector3 d = (pixelgridCenter - cameraPos) + x2 + y2;
				d = normal(d);
				
				//calculate a,b,c
				double a = dot(d,d);
				double b = dot(2*d,cameraPos - spherePosition);
				double c = dot(cameraPos - spherePosition,cameraPos - spherePosition) - sphereRadius;

...
int instinct_main()
{
  ...
  // Output circles wherever circle touches the button
  // the QUIT button.
  while (mousex < 3 or  mousex > 7 or mousey < 3.5 or mousey > 6.5)
  {Point mouse_click = win.get_mouse("Click somewhere. Click the quit button to exit, will also quit if circle touches it");
    mousex = mouse_click.get_x();
    mousey = mouse_click.get_y();
        if(mousex < 3 or  mousex > 7 or mousey < 3.5 or mousey > 6.5)

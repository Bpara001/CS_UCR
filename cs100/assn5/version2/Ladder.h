...
void  set_adj(Vertex &node,  const Dictionary &dict)
{
	...
	for (int i =0; i < s.length(); i++)
	{
		...
		for (char ch = 'a'; ch <= 'z'; ch++)
		{
			...
			if(dict.contains(word) && word != s && !used.search(word))
			{
			    node.children.push_back(Vertex(word));
			    node.children.back().parent = &node;
			    used.insert(word);           // add word to used list
			}
		}
	}
}
...
(const Dictionary &dict,const string &start,Vertex node)
{
    ...
    //enqueue succesors
    for(int i = 0; i < node.children.size(); i++) pq.push(node.children[i]);
...
void print()
{
    ...
    if(!totalused.search(dict.data[i]))
        find_search(dict.data[i]);
    cout << "\n" << dict.size() - (totalused.size() - magic_offset) <<endl;
    cout << dict.size() << " " << totalused.size() << " " << magic_offset << endl;
}
